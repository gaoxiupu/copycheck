// This script is injected into the page to extract text and handle highlights.

{
    // Check if the script's main logic has already been initialized to avoid multiple listeners
    if (!window.copyCheckInjected) {
        window.copyCheckInjected = true;
        ...
        function buildSearchCandidates(location, description) {
            ...
            // 1. Full location text (Highest priority)
            add(location);

            // 2. Location split into segments (Fallback)
            if (location) {
                const segments = location.split(/[，,、；;。！!？?\s]+/).filter(s => s.length >= 2);
                segments.forEach(add);
            }

            // 3. Trailing portion of location (Fallback)
        ...
        function chunkContent(items) {
            ...
            for (const item of items) {
                ...
                if (itemTokens > CHUNK_TOKEN_LIMIT) {
                    ...
                    const charsPerChunk = CHUNK_TOKEN_LIMIT * 2;
